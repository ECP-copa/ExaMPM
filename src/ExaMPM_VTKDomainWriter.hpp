...
#ifndef EXAMPM_VTKDOMAINWRITER_HPP
#define EXAMPM_VTKDOMAINWRITER_HPP
#include <mpi.h>
...
void writeDomain(MPI_Comm comm, int time_step, std::array<double, 6>& domain_vertices, std::string& basename)
{
    ...
    fprintf(file, "\t\t<DataArray type=\"Int32\" Name=\"rank\" NumberOfComponents=\"1\" format=\"ascii\">\n");
    fprintf(file, "%d", rank);
    fprintf(file, "\n\t\t</DataArray>\n");
    fprintf(file, "\t</CellData>\n");
    fprintf(file, "\t<Points>\n");
    fprintf(file, "\t\t<DataArray type=\"Float64\" Name=\"Points\" NumberOfComponents=\"3\" format=\"ascii\">\n");
    for(const double &vert : vertices)
        fprintf(file, "%g ", vert);
    fprintf(file, "\n\t\t</DataArray>\n");
    fprintf(file, "\t</Points>\n");
    fprintf(file, "\t<Cells>\n");
    fprintf(file, "\t\t<DataArray type=\"Int32\" Name=\"connectivity\" NumberOfComponents=\"1\" format=\"ascii\">\n");
    for(const int &conn : connectivity)
        fprintf(file, "%d ", conn);
    fprintf(file, "\n\t\t</DataArray>\n");
    fprintf(file, "\t\t<DataArray type=\"Int32\" Name=\"offsets\" NumberOfComponents=\"1\" format=\"ascii\">\n");
    fprintf(file, "8\n");
    fprintf(file, "\n\t\t</DataArray>\n");
    fprintf(file, "\t\t<DataArray type=\"UInt8\" Name=\"types\" NumberOfComponents=\"1\" format=\"ascii\">\n");
    fprintf(file, "11\n");
    fprintf(file, "\n\t\t</DataArray>\n");
    fprintf(file, "\t</Cells>\n");
    fprintf(file, "</Piece>\n");
